...
用途：这份文档用于指导后续把 `src/components/AIAssistant.ux` 从本地 mock 逻辑改造成真实 AI 调用逻辑。后续实现时，应优先按本文拆分端侧蓝心 3B 文本模型、云端蓝心九问 Agent、第三方云能力和本地规则代码的职责。
...
2. 端侧 3B 先解析用户输入，产出结构化候选结果。
3. 本地规则校验候选结果，并计算冲突、预算、统计等确定性结果。
4. 如果结果完整且置信度足够，展示确认卡片。
5. 如果信息缺失、低置信度、多意图或用户要求建议，再调用云端 Agent。
6. 云端 Agent 返回建议或澄清问题后，仍需本地校验，再展示给用户。
...
| 小票图片识别 | OCR 或多模态云服务 | 第三方专项模型 | 当前端侧文本模型不处理原始图片。 |
...
1. 端侧 3B 失败时，尝试本地规则解析常见表达。
2. 本地规则仍不完整时，询问用户缺失字段。
3. 用户选择深度分析或输入复杂时，调用云端 Agent。
4. 云端 Agent 失败时，提示用户稍后重试，并保留当前输入草稿。
...
3. 增加本地 schema 校验、时间校验和金额校验。
4. 将原来的自动添加改成确认卡片。
5. 接入本地冲突检测、预算检测和上下文包生成。
6. 通过后端代理调用蓝心九问 Bot `2495`，用于复杂建议、多轮追问和历史习惯解释。
7. 将云端返回写入 `ai_extractions` / `ai_insights`，用户确认后再写入 `events` / `transactions`。
8. 增加上云前脱敏、`ai_context_packages` 审计和云端失败兜底。
9. 预留第三方 OCR 或多模态接口。
